...
def editProfile():
    ...
    with psycopg2.connect(DATABASE_URL, sslmode='require') as conn:
        cur = conn.cursor()
        cur.execute("SELECT userId, email, firstName, lastName, city, state, country, phone FROM users WHERE email = %s", (session['email'], ))
...
def updateProfile():
    if request.method == 'POST':
        ...
        with psycopg2.connect(DATABASE_URL, sslmode='require') as con:
                try:
                    cur = con.cursor()
                    cur.execute('UPDATE users SET firstName = %s, lastName = %s, city = %s, state = %s, country = %s, phone = %s WHERE email = %s', (firstName, lastName, city, state, country, phone, email))
...
def register():
    if request.method == 'POST':
        ...
        with psycopg2.connect(DATABASE_URL, sslmode='require') as con:
            try:
                cur = con.cursor()
                cur.execute('INSERT INTO users (password, email, firstName, lastName, city, state, country, phone) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)', (hashlib.md5(password.encode()).hexdigest(), email, firstName, lastName, city, state, country, phone))
                con.commit()
